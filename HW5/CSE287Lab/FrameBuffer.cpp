...
void FrameBuffer::setFrameBufferSize(const int width, const int height) {
	...
	colorBuffer = new GLubyte[width*BYTES_PER_PIXEL*height];//ZZZ
	depthBuffer = new float[width*height];//ZZZ
...
void FrameBuffer::clearColorAndDepthBuffers() {
	...
	for (int y = 0; y < window.height; ++y) {
		for (int x = 0; x < window.width; ++x) {
			int arrayPos = x + y * window.width;
			std::memcpy(colorBuffer + BYTES_PER_PIXEL * arrayPos,
									clearColor, BYTES_PER_PIXEL);
			depthBuffer[arrayPos] = 1.0f;
		}
	}
